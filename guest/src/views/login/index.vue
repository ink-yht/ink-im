...
const checkRememberMe = () => {
    if (form.email) {
        const rememberMe = localStorage.getItem(`rememberMe_${form.email}`) === 'true';
        if (rememberMe) {
            ...
            if (encryptedEmail && encryptedPassword) {
                ...
                const bytesPassword = CryptoJS.AES.decrypt(encryptedPassword, 'secret-key');
...
const handleLogin = () => {
    if (form.email && form.password) {
        if (form.rememberMe) {
            // 加密并存储凭证
            const encryptedEmail = CryptoJS.AES.encrypt(form.email, 'secret-key').toString();
        ...
        <div class="login_form">
            <el-form :model="form">
                ...
                <el-form-item class="item_password">
                    ...
                    <el-input :type="form.showPassword ? 'text' : 'password'" v-model="form.password" placeholder="密码" :prefix-icon="Lock">
                        <!-- 添加一个 slot 来放置密码可见性切换按钮 -->
                        <template #suffix>
                            <i v-if="form.showPassword" style="cursor: pointer; font-size: 18px;"
                                @click="togglePasswordVisibility"><el-icon>
                                    <Hide />
                                </el-icon></i>
                            <i v-else style="cursor: pointer; font-size: 18px;" @click="togglePasswordVisibility"><el-icon>
                                    <View />
                                </el-icon></i>
                        </template>
                    </el-input>
                ...
                <el-form-item class="item_action">
                  ...
                  <el-link :underline="false" type="primary" class="right">注册账号</el-link>
                </el-form-item>
                <el-form-item>
